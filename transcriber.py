...
def transcribe_audio(filename):
    ...
    # Save the transcription to a text file in the "textFiles" folder
    text_folder = "textFiles"
    if not os.path.exists(text_folder):
        os.makedirs(text_folder)
    
    output_text_path = os.path.join(text_folder, f"{os.path.splitext(filename)[0]}_transcription.txt")
...
if __name__ == "__main__":
    ...
    filename = input("Enter the audio file name (including extension) located in the 'audio' folder: ")
